...
function sendChanges(scope, dbname, res, req, seq) {
  return () => {
    let changes = scope.changes[dbname].reduce((list, change) => {
      // Handle include_docs param
    ...
    // let stringy_changes = JSON.stringify(changes)
    // console.log('stringy_changes:', stringy_changes)
    // stringy_changes.forEach((o) => {
    //   res.write(`${o}\n`)
    // })
