...
function save_doc(self) {
  ...
  return (req, res, next) => {
    ...
    if (req.params.hasOwnProperty('designdoc')) {
      doc._id = `_design/${req.params.designdoc}`;
    ...
    if (!current) {
      ...
      res.send(201, { ok: true, id, rev: doc._rev });
    }
    ...
    if (!!current && current._rev === doc._rev) {
      ...
      res.send(201, { ok: true, id, rev: current._rev });
    }

    self.emit(req.route.method, { type: 'document', id, doc });
    next();
  };
}

module.exports = save_doc
